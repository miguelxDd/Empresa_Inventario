...
class ProductoController extends Controller
{
    ...
    public function store(StoreProductoRequest $request)
    {
        try {
            ...
            $data['created_by'] = \Illuminate\Support\Facades\Auth::id() ?? 1; // Agregar usuario autenticado o fallback
    ...
    public function update(UpdateProductoRequest $request, Producto $producto)
    {
        try {
            $data = $request->validated();
            $data['updated_by'] = \Illuminate\Support\Facades\Auth::id() ?? 1; // Agregar usuario que actualiza
    ...
    public function getFormOptions(): JsonResponse
    {
        try {
            $categorias = Categoria::where('activa', true)->orderBy('nombre')->get(['id', 'nombre']);
            $unidades = Unidade::where('activa', true)->orderBy('nombre')->get(['id', 'nombre', 'abreviatura']);
            $cuentas = Cuenta::where('activa', true)->orderBy('codigo')->get(['id', 'codigo', 'nombre']);
